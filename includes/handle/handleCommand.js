module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  // Hàm chọn ảnh ngẫu nhiên từ danh sách ảnh
  function getRandomImageUrl() {
    const images = JSON.parse(fs.readFileSync(path.join(__dirname, './../../includes/datajson/gaivip.json')));
    return images[Math.floor(Math.random() * images.length)].trim();
  }

  // Hàm tải và gửi ảnh
  function downloadAndSendImage(imageUrl, callback) {
  ...
  return async function ({ event }) {
    ...
    if (!command) {
      ...
      if (checker.bestMatch.rating >= 0.5) command = client.commands.get(checker.bestMatch.target);
        ...
        // Tải và gửi ảnh cùng với tin nhắn
        downloadAndSendImage(randomImageUrl, (filePath) => {
          api.sendMessage({
