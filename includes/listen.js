...
(async function () {
  ...
  logger.loader(` NAME BOT: ${(!global.config.BOTNAME) ? "This bot was made by Khôi" : global.config.BOTNAME}`)
  logger.loader("┗━━━━━━━━━━━━━━━━━━━━━━━━━━┛")
...
if ((event.body || '').startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.ADMINBOT.includes(event.senderID)) {
    ...
    if (find_thuebot && new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000 && event.body.trim() !== `${prefix}callad`) {
        if (event && event.threadID) {
