...
;
    if (body.toLowerCase() === "all") {
        for (const singleIndex in handleReply.pending) {
            ...
            api.sendMessage("", event.threadID, () => api.sendMessage(`❯ Bạn đã được duyệt bot bởi Admin: fb.com/61568801032424`, handleReply.pending[singleIndex].threadID));
            count += 1;
        }
        ...
        for (const singleIndex of index) {
            ...
            api.sendMessage("", event.threadID, () => api.sendMessage(`❯ Admin: fb.com/61568801032424`, handleReply.pending[singleIndex - 1].threadID));
            count += 1
        }
