        ...
        switch (o.args[0]) {
            ...
            case 'check': {
                ...
                try {
                    ...
                    const pageKeys = keys.slice(startIndex, endIndex).map((key, i) => {
                        const expirationDate = moment(form_mm_dd_yyyy(key.expirationDate), 'MM/DD/YYYY');
                        const hasExpired = expirationDate.isBefore(now) ? 'Đã Hết Hạn ❌' : 'Chưa Hết Hạn 🔄';
                        return `${i + 1}. Key: ${key.activationKey}\n🗓 Ngày hết hạn: ${key.expirationDate}\n💬 HSD: ${hasExpired}\n🔎 Tình trạng: ${key.used ? 'Đã kích hoạt ✅' : 'Chưa kích hoạt ❌'}\n🏘️ Tên Nhóm: ${key.threadName || 'N/A'}\n👤 Tên Người Thuê: ${key.userName || 'N/A'}`;
                    });
            ...
            case 'expired': {
                ...
                let expiredKeys = keys.filter(key => {
                    let expirationDate = moment(form_mm_dd_yyyy(key.expirationDate), 'MM/DD/YYYY');
                    return expirationDate.isBefore(moment.tz('Asia/Ho_Chi_Minh'));
                });
            
                if (expiredKeys.length === 0) return send("❌ Hiện tại không có key nào đã hết hạn.");
            
                send(`⚠️ Bạn có chắc muốn xóa toàn bộ key đã hết hạn không?\n\nReply 'y' để xóa tất cả key đã hết hạn,'n' để chỉ xóa key chưa kích hoạt đã hết hạn.`);
            
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: o.event.messageID,
                    author: o.event.senderID,
                    expiredKeys
                });
                break;
            ...
            default:
                send(`[ Menu Thuê Bot ]\n──────────────────\n- Dùng: ${prefix}${this.config.name} key [số ngày thuê] [số key muốn tạo]: tạo key ngẫu nhiên với số ngày thuê bot\n- Dùng: ${prefix}${this.config.name} check: xem danh sách các key\n- Dùng: ${prefix}${this.config.name} info: xem thông tin thuê bot của nhóm.\n- Dùng: ${prefix}${this.config.name} update: cập nhật tất cả tên nhóm\n- Dùng: ${prefix}${this.config.name} expired: xoá all key đã hết hạn.`);
                break;
        }
    } catch (e) {
        console.log(e);
...
exports.handleReply = async function(o) {
    try {
        ...
        switch (o.handleReply.name) {
            case 'chooseyn':
                if (o.event.body.toLowerCase() == 'y') {
                    let expiredKeys = o.handleReply.expiredKeys;
                    let deletedKeys = expiredKeys.length;
                    
                    keys = keys.filter(key => !expiredKeys.includes(key));
                    saveKeys();
                    
                    send(`✅ Đã xóa ${deletedKeys} key đã hết hạn bao gồm đã và chưa kích hoạt.`);
                } else if (o.event.body.toLowerCase() == 'n') {
                    let expiredKeys = o.handleReply.expiredKeys.filter(key => !key.used);
                    let deletedKeys = expiredKeys.length;
    
                    keys = keys.filter(key => !expiredKeys.includes(key));
                    saveKeys();
                    
                    send(`✅ Đã xóa ${deletedKeys} key chưa kích hoạt đã hết hạn.`);
                }
                break;
        }
